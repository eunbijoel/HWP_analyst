...
6. **검토 이슈 블록이 있으면** expected/actual/difference·rule_id·위치를 그대로 인용하세요. 숫자를 다시 계산하거나 고치지 말고, 왜 플래그됐는지와 표에서 확인할 칸·수정 체크리스트만 안내하세요.
7. 계산이 필요하면 과정을 단계별로 보여주세요.
8. 확실하지 않으면 "문서에서 명확히 확인되지 않음"이라고 표시하세요.
